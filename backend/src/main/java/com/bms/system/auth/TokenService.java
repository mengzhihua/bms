 ...
 * 密钥来自 bms.auth.secret（环境变量 BMS_AUTH_SECRET）；未配置时每次启动随机生成，重启后旧令牌全部失效。
 */
@Slf4j
@Component
public class TokenService {
    ...
    public TokenService(@Value("${bms.auth.secret:}") String secret,
                        @Value("${bms.auth.token-ttl:12h}") Duration ttl) {
        if (secret == null || secret.trim().isEmpty()) {
            byte[] random = new byte[32];
            new SecureRandom().nextBytes(random);
            this.key = random;
            log.warn("bms.auth.secret 未配置，使用随机密钥，应用重启后需要重新登录");
        } else {
