...
chcp 65001 >nul
cd /d "%~dp0"
if not defined SERVER_PORT set SERVER_PORT=8084
...
echo 正在启动 BMS 计费管理系统
echo 浏览器打开 http://127.0.0.1:%SERVER_PORT%
echo 服务端也可直接：java -jar bms-backend-1.0.0.jar --server.port=%SERVER_PORT%

if not "%SKIP_BROWSER%"=="1" (
  start "" cmd /c "timeout /t 8 /nobreak >nul && start http://127.0.0.1:%SERVER_PORT%"
)

java %JAVA_OPTS% -jar bms-backend-1.0.0.jar --server.port=%SERVER_PORT%
